$(function(){
    ...
    $("#txtAmount1").keyup(function(event){
        {
            event.preventDefault();
            var crptoSymbol = $("select#selectCurrency2").children("option:selected").attr("data-symbol");
            var price = $("select#selectCurrency2").children("option:selected").attr("data-price");
            var amount = $(this).val();
            var quantity = amount/price;
            $("#txtQuanity1").val(quantity.toFixed(6));
        }
    });


    $("#txtQuanity1").keyup(function(event){
        {
            var crptoSymbol = $("select#selectCurrency2").children("option:selected").attr("data-symbol");
            var price = $("select#selectCurrency2").children("option:selected").attr("data-price");
            var quantity  = $(this).val();
            var amount=  price *quantity;
            $("#txtAmount1").val(amount.toFixed(2));
        }
    });
    ...
    $('#btnBuyToken1').click(function(event){
        event.preventDefault();
        var crptoSymbol = $("select#selectCurrency2").children("option:selected").attr("data-symbol");
        var price = $("select#selectCurrency2").children("option:selected").attr("data-price");
        var amount = $("#txtAmount1").val();
        var quantity = $("#txtQuanity1").val();
    ...
    $('.btnBuyTb').on('click', function(){
        var symbol = $(this).attr("data-symbol");
        //task is to show the modal and set the selected option to the corresponding symbol
        $("#selectCurrency2").val(symbol);
        $("#BuyModal").modal();
